    ...
    def ejecutar(self):
        ...
        match nombre_algoritmo.lower():
            ...
            case "evolutivogeneracional":
                ...
                procruce=int(self.parametros[15])
                logging.info(f"\t\tProcruce:\t{procruce}")
                promut=int(self.parametros[16])
                ...
                start_time = time.perf_counter()
                logger=logging
                algoritmo = self.ejecutar_algoritmo(nombre_algoritmo,logging=logger, matriz_distancias=matriz_d, k=k, seed=seed,
