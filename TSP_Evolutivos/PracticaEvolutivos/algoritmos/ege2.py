...
class ege2:
    ...
    def ejecutar(self):
        ...
        # Obtenemos los élites iniciales
        ...
        while evaluaciones < self.Evmax and (time.time() - inicio) < self.Tmax:
            ...
            # Cruce: Lista de descendientes que contendrá la siguiente generación en estado base
            descendientes = []
            ...
            # Reemplazamiento: Reemplazamos totalmente la generación anterior por la nueva
            poblacion_nueva = descendientes

            # Elitismo:
            # Comprobamos si los élites actuales están en la nueva población
            for elite in elites:
                if elite not in poblacion_nueva:
                    # Reemplazamos al peor individuo por el élite
                    peor = max(poblacion_nueva, key=lambda ind: ind['fitness'])
                    poblacion_nueva.remove(peor)
                    poblacion_nueva.append(elite)

            # Actualizamos el mejor individuo global
            ...
            # Actualizamos los élites para la próxima iteración
            poblacion_ordenada = sorted(poblacion_nueva, key=lambda ind: ind['fitness'])
            elites = poblacion_ordenada[:self.num_elites]

            # Actualizamos la población para la siguiente generación
            poblacion = poblacion_nueva

        # Cuando se cumplen las condiciones de parada, devolvemos el mejor individuo obtenido
        return mejor_global
